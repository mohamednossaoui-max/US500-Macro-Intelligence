...
DATASETS: Dict[str, Dict[str, Any]] = {
    ...
    "Liquidity": {
        ...
        "files": ["liquidity_intelligence_v1.csv", "liquidity_intelligence_research_v1.csv"],
    },
    "Market Breadth": {
...
st.markdown(
    """
<style>
.stApp{background:#070a0f;color:#e8edf5}
section[data-testid="stSidebar"]{background:#090d13}
.block-container{max-width:1550px;padding-top:1rem;padding-bottom:4rem}
.hero{background:linear-gradient(135deg,#111823,#090d14);border:1px solid #202b3b;border-radius:18px;padding:27px 30px;margin-bottom:18px}
.hero-title{font-size:31px;font-weight:780;letter-spacing:-.035em}
.hero-subtitle{color:#7e8a9c;margin-top:5px;font-size:13px}
...
.badge{border:1px solid #263446;background:#121a25}
.research-badge{border:1px solid #245046;background:#0d1c19;color:#6ed8c1}
.section{background:#0d121a;border:1px solid #202b39;border-radius:15px;padding:19px;margin-bottom:17px}
.section-title{font-size:17px;font-weight:730;margin-bottom:3px}
.section-subtitle{color:#7e8a9c;font-size:11px;margin-bottom:15px}
.metric-card{background:linear-gradient(145deg,#111821,#0d131b);border:1px solid #202b39;border-radius:14px;padding:17px;min-height:106px}
.metric-label{color:#778498;text-transform:uppercase;font-size:10px;letter-spacing:.09em}
.metric-value{margin-top:8px;font-size:25px;font-weight:760}
.metric-sub{color:#697688;margin-top:4px;font-size:11px}
.state-card{border:1px solid #243042;background:#0e151f;border-radius:12px;padding:14px;min-height:90px}
.state-label{color:#758296;font-size:10px;text-transform:uppercase;letter-spacing:.08em}
.state-value{font-size:16px;font-weight:720;margin-top:7px}
.state-source{color:#667386;font-size:10px;margin-top:4px}
.evidence{background:#0b1118;border-left:3px solid #405168;border-radius:7px;padding:12px 14px;margin-bottom:9px}
.evidence-title{font-weight:700;font-size:12px}
.evidence-text{color:#aab5c4;font-size:11px;line-height:1.55;margin-top:3px}
.warning-box{background:#19150d;border:1px solid #4b3b1e;border-radius:10px;padding:13px;color:#cdbd95;font-size:11px}
.info-box{background:#0c141d;border:1px solid #233348;border-radius:10px;padding:13px;color:#9eabbc;font-size:11px}
.success-box{background:#0c1915;border:1px solid #245046;border-radius:10px;padding:13px;color:#82d8c2;font-size:11px}
.footer{color:#586577;text-align:center;font-size:10px;padding-top:30px}
</style>
""",
...
def fmt(value: Any, digits: int = 2) -> str:
    x = number(value)
    return clean(value) if x is None else f"{x:,.{digits}f}"
...
def latest_date(df: Optional[pd.DataFrame]) -> Optional[pd.Timestamp]:
    if df is None or df.empty:
        return None
    col = find_column(df, ["context_date","asof_date","study_date","observation_date","date","event_date","reported_date"])
    if col is None:
        return None
...
def github_headers() -> Dict[str, str]:
    headers = {
        ...
        "User-Agent": "US500-Research-Terminal/4.1",
...
def github_artifact_by_name(name: str) -> Optional[Dict[str, Any]]:
    url = (
    ...
    try:
        ...
        if response.status_code == 404:
            ARTIFACT_ERRORS[name] = "GitHub returned 404."
            return None
        if response.status_code == 403:
            remaining = response.headers.get("X-RateLimit-Remaining", "unknown")
            ARTIFACT_ERRORS[name] = f"GitHub API returned 403. Rate-limit remaining: {remaining}."
            return None
        if response.status_code == 429:
            ARTIFACT_ERRORS[name] = "GitHub API rate limit exceeded."
            return None
        ...
        if not valid:
            ARTIFACT_ERRORS[name] = f"No non-expired artifact found: {name}"
            return None
        valid.sort(key=lambda x: (x.get("created_at", ""), x.get("id", 0)), reverse=True)
        return valid[0]
    except requests.RequestException as exc:
        ARTIFACT_ERRORS[name] = f"GitHub lookup failed: {type(exc).__name__}: {exc}"
        return None
    except Exception as exc:
        ARTIFACT_ERRORS[name] = f"Unexpected lookup error: {type(exc).__name__}: {exc}"
        return None
...
def latest_from_candidates(names: List[str]) -> Optional[Dict[str, Any]]:
    candidates = [a for name in names if (a := latest_artifact(name))]
    if not candidates:
        return None
...
def download_artifact(artifact_id: int) -> Dict[str, bytes]:
    url = f"{GITHUB_API}/actions/artifacts/{artifact_id}/zip"
...
def artifact_files(artifact_name: Optional[str] = None, artifact_names: Optional[List[str]] = None) -> Dict[str, bytes]:
    ...
    if artifact is None:
        ARTIFACT_ERRORS.setdefault(label, "Artifact was not found or could not be accessed.")
        return {}
    try:
...
def load_named_csv(dataset_name: str, file_index: int = 0) -> Optional[pd.DataFrame]:
    ...
    filenames = [f for f in config.get("files", []) if f.lower().endswith(".csv")]
    if file_index >= len(filenames):
        return None
    files = artifact_files(config.get("artifact"), config.get("artifact_candidates"))
    raw = find_artifact_file(files, [filenames[file_index]])
    if raw is None:
        DATASET_ERRORS[dataset_name] = f"File not found in artifact: {filenames[file_index]}"
        return None
    df = safe_read_csv(raw)
    if df is None or df.empty:
        DATASET_ERRORS[dataset_name] = f"CSV unavailable or empty: {filenames[file_index]}"
        return None
    return df
...
def load_named_json(dataset_name: str) -> Optional[Dict[str, Any]]:
    config = DATASETS.get(dataset_name, {})
    names = [f for f in config.get("files", []) if f.lower().endswith(".json")]
    if not names:
        return None
    files = artifact_files(config.get("artifact"), config.get("artifact_candidates"))
    raw = find_artifact_file(files, names)
    if raw is None:
        return None
    try:
...
def load_dataset(name: str) -> Optional[pd.DataFrame]:
    ...
    if df is not None and not df.empty:
        return df
    for filename in DATASETS.get(name, {}).get("files", []):
...
earnings_detail_files = load_named_csvs("Earnings")
historical_files = load_named_csvs("Historical Edge")
final_validation_files = load_named_csvs("Final Validation")

def canonical_value(names: List[str], default: Any = None) -> Any:
    return row_value(research_context, names, default)

CTX = {
    "date": canonical_value(["context_date","asof_date","study_date"]),
    "layers": canonical_value(["available_layer_count","layer_count"]),
    "pit": canonical_value(["point_in_time_safe"]),
    "economic_regime": canonical_value(["macro_economic_regime","economic_regime","research_regime"]),
    "financial_stress_regime": canonical_value(["macro_financial_stress_regime","financial_stress_regime","stress_regime","research_regime"]),
    "sentiment_regime": canonical_value(["sentiment_research_regime","sentiment_regime","research_regime"]),
    "technical_regime": canonical_value(["technical_technical_regime","technical_regime","research_regime"]),
}
...
def state_card(label: str, value: Any, source: str = "") -> None:
    st.markdown(
        ...
        <div class="state-source">{source}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def dataset_status(name: str, df: Optional[pd.DataFrame]) -> None:
    if df is None or df.empty:
        warning(f"<strong>{name}</strong><br>Dataset unavailable.")
    else:
...
def render_regime() -> None:
    ...
    cols = st.columns(4)
    items = [
        ("Macro regime", CTX["economic_regime"], "Research Context"),
        ("Stress regime", CTX["financial_stress_regime"], "Research Context"),
        ("Sentiment regime", CTX["sentiment_regime"], "Research Context"),
        ("Technical regime", CTX["technical_regime"], "Research Context"),
    ]
    for col, (label, value, source) in zip(cols, items):
        with col:
            state_card(label, value or "Not available", source)
    st.markdown("### Research integrity")
    evidence("Research date", f"<strong>{date_value(CTX['date'])}</strong>")
    evidence("Available layers", f"<strong>{fmt(CTX['layers'], 0)}</strong>")
    evidence("Point-in-time safe", f"<strong>{clean(CTX['pit']) or 'N/A'}</strong>")

def render_overview() -> None:
    hero("US500 Research Intelligence", "Consolidated view of the latest research state.", "Overview")
    cols = st.columns(4)
    items = [
        ("Macro", CTX["economic_regime"], "Macro research"),
        ("Financial stress", CTX["financial_stress_regime"], "Financial Stress"),
        ("Sentiment", CTX["sentiment_regime"], "Sentiment research"),
        ("Technical", CTX["technical_regime"], "Technical research"),
    ]
    for col, (label, value, source) in zip(cols, items):
        with col:
            state_card(label, value or "Not available", source)
    ...
    with section:
        ...
        info(
            "The dashboard is research-only. It does not generate trading signals, "
            "forecasts, execution instructions, position sizing, or SL/TP."
        )
        evidence("Research date", date_value(CTX["date"]))
        evidence("Available layers", fmt(CTX["layers"], 0))
...
def render_macro() -> None:
    ...
    if macro_context is not None:
        table_clean(macro_context, 30)

def render_stress() -> None:
    hero("Financial Stress", "Market stress, volatility, rates and financial conditions.", "Financial Stress")
    dataset_status("Financial Stress", financial_stress)
    if financial_stress is not None:
        table_clean(financial_stress, 30)

def render_sentiment() -> None:
    hero("Sentiment Intelligence", "Observed sentiment evidence.", "Sentiment")
    dataset_status("Sentiment", sentiment)
    if sentiment is not None:
        table_clean(sentiment, 30)
...
def render_technical() -> None:
    ...
    if technical is not None:
        table_clean(technical, 30)

def render_fed() -> None:
    hero("Fed Intelligence", "FOMC, minutes, press conference and SEP evidence.", "Fed Intelligence")
    if not fed_intelligence:
        warning("Fed Intelligence artifact is unavailable.")
        return
    st.json(fed_intelligence, expanded=False)

def render_liquidity() -> None:
    hero("Liquidity", "Observed liquidity conditions.", "Liquidity")
    dataset_status("Liquidity", liquidity)
    table_clean(liquidity, 40)

def render_breadth() -> None:
    hero("Market Breadth", "Breadth participation and market structure.", "Market Breadth")
    dataset_status("Market Breadth", market_breadth)
    table_clean(market_breadth, 40)

def render_earnings() -> None:
    hero("Earnings", "Observed market reactions around earnings events.", "Earnings")
...
def render_historical_edge() -> None:
    hero("Historical Edge", "Comparable historical environments.", "Historical Edge")
    if not historical_files:
        warning("Historical Event Study artifact is unavailable.")
        return
...
def render_event_studies() -> None:
    hero("Event Studies", "Historical event-response evidence.", "Event Studies")
...
def render_cross_asset() -> None:
    ...
    table_clean(cross_asset, 50)

def render_evidence() -> None:
    hero("Evidence", "Current-state evidence and validation.", "Evidence")
    evidence("Macro", clean(CTX["economic_regime"]) or "Not available")
    evidence("Financial stress", clean(CTX["financial_stress_regime"]) or "Not available")
    evidence("Sentiment", clean(CTX["sentiment_regime"]) or "Not available")
    evidence("Technical", clean(CTX["technical_regime"]) or "Not available")
...
def render_data_status() -> None:
    hero("Data Status", "Deployment diagnostics for GitHub Actions research artifacts.", "Data Status")
    if secret("GITHUB_TOKEN"):
        success("GITHUB_TOKEN detected. Authenticated access is enabled.")
    else:
        info("No GITHUB_TOKEN configured. Public unauthenticated GitHub API access is being used.")
    rows = []
    for name, config in DATASETS.items():
        ...
        rows.append({
            ...
            "Latest": date_value(latest_date(df)) if df is not None else "N/A",
            "Status": "AVAILABLE" if df is not None and not df.empty else "MISSING",
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    if ARTIFACT_ERRORS:
        st.markdown("### Artifact diagnostics")
        for name, error in ARTIFACT_ERRORS.items():
            warning(f"<strong>{name}</strong><br>{error}")
    if DATASET_ERRORS:
...
with st.sidebar:
    st.markdown("## ◈ US500")
    st.caption("Research Intelligence Terminal")
    page = st.radio("Research", list(PAGES.keys()), index=0)
    st.markdown("---")
    st.caption(f"Version {APP_VERSION}")
    st.caption(f"Research date: {date_value(CTX['date'])}")

try:
    PAGES[page]()
except Exception as exc:
    warning(f"<strong>The selected page could not be rendered:</strong> {type(exc).__name__}: {exc}")

st.markdown(
    '<div class="footer">US500 Macro Intelligence · Research-only visualization</div>',
    unsafe_allow_html=True,
)
