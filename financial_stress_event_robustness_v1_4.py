...
OUTPUT_SUMMARY = "financial_stress_event_robustness_summary_v1_4.csv"
OUTPUT_EVENTS = "financial_stress_event_robustness_events_v1_4.csv"

START_THRESHOLD = 1.0
EXIT_THRESHOLDS = [0.00, 0.25, 0.50]


def detect_events(score: pd.Series, exit_threshold: float) -> pd.DataFrame:
    s = score.dropna().sort_index()
    events = []
    i = 0

    while i < len(s):
        if s.iloc[i] < START_THRESHOLD:
            i += 1
            continue

        start_i = i
        j = i
        while j + 1 < len(s) and s.iloc[j + 1] >= exit_threshold:
            j += 1
        ...
        for k in range(j + 1, len(s)):
            if s.iloc[k] < exit_threshold:
                recovery_date = s.index[k]
                break

        events.append({
            "exit_threshold": exit_threshold,
            "start_date": s.index[start_i].date().isoformat(),
            "end_date": s.index[j].date().isoformat(),
            "peak_date": peak_date.date().isoformat(),
            "peak_composite": float(segment.max()),
...
def main():
    ...
    session_dates = pd.DatetimeIndex(sorted(
        df.loc[
    ...
    # daily = 252 native observations; weekly = 52 native observations.
    ...
    events = pd.concat(
        [detect_events(score, threshold) for threshold in EXIT_THRESHOLDS],
        ignore_index=True,
    )

    summary_rows = []
    for threshold in EXIT_THRESHOLDS:
        e = events[events["exit_threshold"] == threshold]
        summary_rows.append({
            "methodology_version": "v1.4",
            "base_method": "F_FREQUENCY_AWARE_v1.3",
            "start_threshold": START_THRESHOLD,
            "exit_threshold": threshold,
            "scored_sessions": int(score.notna().sum()),
            "event_count": int(len(e)),
            "longest_event_sessions": int(e["duration_trading_sessions"].max()) if len(e) else 0,
            "largest_peak": float(e["peak_composite"].max()) if len(e) else np.nan,
            "research_only": True,
        })
    ...
    print("Financial Stress Event Robustness Review v1.4 completed.")
