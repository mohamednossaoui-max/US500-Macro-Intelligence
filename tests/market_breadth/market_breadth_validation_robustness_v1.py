...
def main():
    ...
    # Recompute the 252-observation expanding history using data through T only.
    # min_periods=252 matches the analyzer's warm-up convention.
    z_cols = [
    ...
    for raw_col, z_col in z_cols:
        ...
        recomputed = (
            raw.rolling(window=252, min_periods=252)
               .apply(lambda x: (x.iloc[-1] - x.mean()) / x.std(ddof=1)
    ...
    expected_warmup = max(0, len(df) - 251)
    actual_z = int(df["net_advances_z_252d"].notna().sum())
    if len(df) >= 252 and actual_z != expected_warmup:
        warnings.append(
            f"Unexpected 252D z-score warm-up count: expected {expected_warmup}, got {actual_z}."
        )
